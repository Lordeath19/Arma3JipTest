	...
	_loopWhitelist = [] spawn {
		...
		while {Admin_whiteListEnabled} do {{
				If ((getPlayerUID _x) in Admin_whitelist) then {
					[["BACK-TO-LOBBY"],
					{ ["LOSER",false,0,true] spawn BIS_fnc_endMission }] remoteExec ["spawn",_x]}} forEach (allPlayers select {(str (getPlayerUID _x)) in Admin_whitelist})}};
comment "----------------------------"; 
...
comment "----------------------------"; 
	_introMusic = [] spawn {
		...
		playMusic "" };
	typeWriter = [ [
			["Release Admin Menu Version 1.6 Alpha","<t align = 'center' ,<br\>,size='10' color='#00FF00'>%1</t><br/>"],
			["Client Initialized","<t align = 'center' <br\>,size ,color='#ff5134'= '2'>%1</t><br/>"],
			["Use Admin RESPONSIBLY.","<t align = 'center' <br\>,size='2' color='#42D6FC'>%1</t>"]]] spawn BIS_fnc_typeText;
	[] spawn { 
		hint "Welcome to Admin Menu V1.6A";
		sleep 0.5;
		hint "									    ";
		sleep 0.3;
		hint "Welcome to Admin Menu V1.6A";
		...
		hint "									    ";
		sleep 0.3;
		hint "-------\nAdmin Menu Keybinds\n---------\nNumber Pad 5 - Main Menu\nINS - Arsenal Menu\nH - 3D Teleport\n-----------------" } comment "Post-Process Effects";
	[] spawn {
		_startEffect = ["ChromAberration", 200, [0.1, 0.1, true]] spawn { params ["_name", "_priority", "_effect", "_handle"] while {
				_handle = ppEffectCreate [_name, _priority];
				_handle < 0 } do { _priority = _priority + 1 };
		...
		_stopEffect = ["ChromAberration", 200, [0, 0, true]] spawn {
			...
			ppEffectDestroy _handle }} comment "Init Done Message";
	[] spawn { waitUntil { scriptDone typeWriter } playSound3D ["A3\Sounds_F_patrol\radio\combatPatrol\02_mission_start_a.ogg", player, false]};
comment "---------------------------"; 
...
comment "---------------------------"; 
	Admin_fnc_mutePlayer = {
		...
		{ if ( ( name _x ) == _playerName ) then {
			...
			[[],{
				for [{_i=0}, {_i<5}, {_i=_i+1}] do { _i enableChannel false }}] remoteExec ["spawn", _x]}} forEach allPlayers };
	Admin_fnc_unmutePlayer = { params ["_playerName"] _playerName = _this select 0;
		{if ( ( name _x ) == _playerName ) then {
			...
			[[],{ for [{_i=0}, {_i<5}, {_i=_i+1}] do {_i enableChannel true}}] remoteExec ["Spawn", _x];}} forEach allPlayer};
	Admin_fnc_autoKickPlayer = { nameOfTroll = _this select 0 comment "Identify Target";
		{ if ((name _x) == nameOfTroll) then {banTarget = _x }} forEach allPlayers comment "Create BAN Function";
		Admin_banThisTroll = {
			...
			[[banReason],{
				...
				[[nameOfTroll, steamIDofTroll],{
					...
					_mouseDetection3 ctrlCommit 0 }] remoteExec ["spawn", remoteExecutedOwner]}] remoteExec ["spawn", banTarget, str (getPlayerUID banTarget)]};
	...
	Admin_fnc_experimentalBan = {
		...
		{if ( ( name _x ) == _playerName ) then {
			...
			[[( name _x )],{
				...
				_mouseDetection ctrlCommit 0 }] remoteExec ["spawn",_x] }} forEach allPlayers };
	Admin_fnc_whitelistPlayer = { params["_playerName"] _playerName = _this select 0;
		{ if (_playerName find (name _x) > -1) then { Admin_whitelist pushBackUnique str (getPlayerUID _x) }} forEach allPlayers;
		_textNotif = format ["Admin: Adding %1 to the White List by SteamID...", _playerName];
		[[_textNotif, "PLAIN DOWN", 0, true, false]] remoteExec ["cutText", 0]};
	Admin_fnc_freezePlayer = {
		params["_playerName"];
		_playerName = _this select 0;
		if (_playerName == Admin_myName) exitWith {hint "Admin: Do not freeze yourself, and if you do you are retarded!"};
			{if ( ( name _x ) == _playerName ) then { [[( name _x )], { 
				if (!userInputDisabled) then {
					...
					hint format ["%1, your input has been re-enabled by the server Administrator (Admin). Stop being retarded or too loud!!!",(_this select 0)];
					}] remoteExec ["spawn",_x]}} forEach allPlayers};
	Admin_fnc_TP_selfToPlayer = {
		...
		{if ((name _x) == (_selectedPlayerName)) then {
			...
			{player moveInAny (vehicle _x)} _endPos = getPos _x}} forEach allPlayers;
	...
	Admin_fnc_TP_allToSelf = {
		...
		{[[Admin_myCurrentPosition,Admin_myName],{
			...
			sleep 1;
			player setPos _pos;
			(format ["Admin: Player %1 has teleported to %2.",(name player), _name]) remoteExec ["systemChat",0]}] remoteExec ["spawn",_x]} forEach allPlayers - AllCurators};
	Admin_fnc_assignGameMod = { 
		...
		[[_textNotif, "PLAIN DOWN", 0, true, false]] remoteExec ["cutText", 0];
		{ if ((name _x) == _specifiedPlayerName) then { VVN_x2 = (vehicleVarName _x);
			(format ["Admin: Player %1 has been selected as the new Zeus (Game Moderator Slot).",(name _x)]) remoteExec ["systemChat",0];
			[["<t color='#42D6FC'>YOU ARE NOW ZEUS (Game Moderator)</t><br/><t color='#58D68D'>-Press [Y] to open/close curator interface-</t>", "PLAIN", -1, true, true]] remoteExec ["titleText", _x]}} forEach allPlayers;
	...
	Admin_fnc_assignGameMaster = {
		...
		{if ((name _x) == _specifiedPlayerName) then {
			...
			[["<t color='#42D6FC'>You Are Now Game Master (Zeus)!</t><br/><t color='#58D68D'>-Press [Y] to open/close curator interface-</t>", "PLAIN", -1, true, true]] remoteExec ["titleText", _x]}} forEach allPlayers;
	...
	Admin_fnc_transferAdmin = {
		...
		[[newAdmin],
		{
			...
			_mouseDetection2 ctrlSetBackgroundColor [0,0,0,0] _mouseDetection2 ctrlCommit 0 }] remoteExec ["spawn",-2];
		(format ["Admin: Administration has been passed from %1 to %2.",Admin_myName,_specifiedPlayerName]) remoteExec ["systemChat",0];
		{[[], {hint "Press Y to re-open Zeus interface."}] remoteExec ["spawn",_x]} forEach allCurators };
	Admin_fnc_execLocal = {
		...
		_result = "";
		_result = [] call _code };
	Admin_fnc_execGlobal = {
		_text = ctrlText console_inputBox;
		if (_text isEqualTo "") exitWith { hint "No code to execute." };
		_code = compile _text;
		_result = _code remoteExec ["bis_fnc_call", 0, false]};
	Admin_fnc_execServer = {
		_text = ctrlText console_inputBox;
		if (_text isEqualTo "") exitWith {hint "Admin: Console Error: No code to execute.};
		_code = compile _text;
		_result = _code remoteExec ["bis_fnc_call", 2, false]};
	Admin_fnc_execPlayer = { params ["_selectedPlayer", "_specifiedName", "_specifiedUID"];
		...
		if (!tNameSelected && !tUIDSelected) then {{
			if (name _x == _selectedPlayer) then {
				_textNotif = format ["Admin: Executing code on %1s client...", (name _x)];
				cutText [_textNotif, "PLAIN DOWN", 0, true, false];
				_result = consoleCode1 remoteExec ["bis_fnc_call", _x, false]}} forEach allPlayers};
		...
		if (tUIDSelected) then {{
			if (str (getPlayerUID _x) == _specifiedUID) then {
				...
				cutText [_textNotif, "PLAIN DOWN", 0, true, false];
				_result = consoleCode1 remoteExec ["bis_fnc_call", _x, false]}} forEach allPlayers}};
	Admin_fnc_giveArsenal = {
		params["_playerName"];
		_playerName = _this select 0;
		{ if ((name _x) == _playerName) then {
			[["Arsenal_Scroll_Option"],
			{ action_openArsenal = player addAction ["Admin: <t color='#42D6FC'>Open Arsenal (1x)</t>", {
...
comment "----------------------------"; 
	Admin_gui_LoadVeh = {
		...
		comment "onLBSelChanged  = [] spawn Admin_gui_VehInfo;";
		...
		for "_x" from 0 to (_count-1) do {
			...
			if (isClass _veh) then { 	if (getNumber (_veh >> "Scope") == 2) then { _class = configName _veh;
					if (_kind == "Car") then { 	if ((_class isKindOf "Car") || (_class isKindOf "Tank")) then {
					...
					} else { if (_class isKindOf _kind) then {
							_index = fSM_RscVehList lbAdd(getText(configFile >> "cfgVehicles" >> _class>> "displayName"));
							fSM_RscVehList lbSetData[(lbSize fSM_RscVehList)-1,  _class];
							_picture = (getText(configFile >> "cfgVehicles" >> _class >> "picture"));
							fSM_RscVehList lbSetPicture[(lbSize fSM_RscVehList)-1,_picture]}}}}}};
	Admin_gui_VehInfo = {private ["_weapArray","_class","_crewCount","_model","_maxSpeed","_invSpace","_armor"];
		...
		{	_name = getText (configFile >> "cfgWeapons" >> _x >> "displayName");
			_weapons = _weapons + [ _name]} forEach _weaponsClass; 
		...
		} else { _weapArray = getArray(configFile >> "cfgVehicles" >> _class >> "Turrets" >> "MainTurret" >> "weapons");
		...
		{	 _name = getText (configFile >> "cfgWeapons" >> _x >> "displayName") _weapons = _weapons + [ _name]} forEach _weapArray;
	...
	Admin_gui_VehCreate = {
		...
		if (count _emptyPos == 0) then { hint "Admin: Error - Either vehicle cannot be spawned due to a lack of space, or you have not selected anything.";
		} else { if (SpawnWithAI) then { fSM_createdVeh = [_emptyPos, (direction player), _classVeh, (side player)] call BIS_fnc_spawnVehicle;
				createVehicleCrew (fSM_createdVeh select 0);
				comment "add vehicle to Zeus interface";
				anal_cream = [fSM_createdVeh];
				[[anal_cream],{_myCockIsBleeding = _this select 0;
					{ _x addCuratorEditableObjects [_myCockIsBleeding,true]} forEach allCurators}] remoteExec ["spawn", 2];
			} else {fSM_createdVeh = createVehicle [_classVeh, _emptyPos, [], 0,""];
				fSM_createdVeh setDir (direction player);
				if (SpawnInVehicle) then {player moveInAny fSM_createdVeh;};
				comment "add vehicle to Zeus interface";
				anal_cream = [fSM_createdVeh];
				[[anal_cream],{ _myCockIsBleeding = _this select 0 {_x addCuratorEditableObjects [_myCockIsBleeding,true]} forEach allCurators}] remoteExec ["spawn", 2]}} Admin_d_fullSpawnMenu closeDisplay 0 };
	Admin_fnc_disableFatigueGlobal = {if (isNil "fatigueTgglGlobal") then {fatigueTgglGlobal = 1;};
         	if (fatigueTgglGlobal == 1) then {comment "Init Start";
			[["InfStam"],
		...
		comment "Init Done" 
	fatigueTgglGlobal = 0;} else {comment "Toggle Off";
	...
	Admin_fnc_show3DPlayerNames = { if (isNil "shw3DPlrNmTggl") then {shw3DPlrNmTggl = 1;}	if (shw3DPlrNmTggl == 1) then {comment "Init Start" [["shw3DPlrNmJIP"],
	...
	Admin_open_utils = {hint "Nothing to see here yet."};
	Admin_toggle_whiteListing = {if (!Admin_whiteListEnabled) then {Admin_whiteListEnabled = true hint "Admin: Whitelist Enabled.";
	...
	Admin_fnc_AASJIP = {if (isNil "AASJIPTggl") then {AASJIPTggl = 1};
	...
	Admin_fnc_viewDistance = { removeAllActions player;
	...
	Admin_fnc_deleteAll = { [["Admin: Deleting all objects on server...", "PLAIN DOWN", 0, true, false]] remoteExec ["cutText", 0] _countObjects = 0;
	...
	Admin_fnc_3DTP = { (vehicle player) setPos (screenToWorld [0.5, 0.5])};
	Admin_open_arsenalMenu = { disableSerialization d_arsenalMenu = (findDisplay 46) createDisplay "RscDisplayEmpty" showChat true; comment "Fixes Chat Bug";
	...
	Admin_fini_fnc_plrs = { _players = [] _all = player nearEntities [['Man','Land','Air','Ship'], 25000];
		{if ((_x isKindOf "Man") && (getPlayerUID _x != "")) then { _players pushBack _x;
	...
	Admin_fini_fnc_hostileAI = { _hostileai = [] { if ((_x isKindOf "Man") && (side _x != side player)) then { _hostileai pushBack _x;
            } else {
                if ((count crew _x) != 0) then { for "_i" from 0 to (count crew _x)-1 do { _l = (crew _x) select _i;
           	if (side _l != side player) then { _hostileai pushBack _l;
                }}}}} forEach allUnits - allPlayers _hostileai};
	Admin_fini_fnc_compile = { (with missionNamespace do compile (_this select 0))};
	Admin_fnc_unflip = { params ["_vehicle"] _vehicle = _this _pos = getPosATL _vehicle pos set [2, 7] _vehicle setPosATL _pos _vehicle setVectorUp [0,0,1]};
comment "----------------------------"; 
...
comment "----------------------------"; 
	Admin_open_optionsPanel = { comment "GUI START" disableSerialization d_optionsPanel = (findDisplay 46) createDisplay "RscDisplayEmpty" showChat true comment "Fixes Chat Bug" comment "Create Controls";
...
comment "Server Menu OLD";
	Admin_deleteMenu = { removeAllActions player _dR = 0;
		player addAction ["Admin: <t color='#BDBDBD'>[CANCEL]</t>", { removeAllActions player }];
		player addAction ["Admin: <t color='#BDBDBD'>[RETURN]</t>", { removeAllActions player [] spawn Admin_open_serverMenu }];
	...
	Admin_deleteRadius = { {deleteVehicle _x;} forEach nearestObjects [player, ["all"], _dR] playSound "Hint" };
	Admin_killMenu = { removeAllActions player _kR = 0;
		player addAction ["Admin: <t color='#BDBDBD'>[CANCEL]</t>", { removeAllActions player }];
		player addAction ["Admin: <t color='#BDBDBD'>[RETURN]</t>", { removeAllActions player [] spawn Admin_open_serverMenu }];
	...
	Admin_killRadius = { { _x setDamage 1; } forEach nearestObjects [player, ["man"], _kR] playSound "Hint" };
	Admin_killAll = { private["i","v","_case","_pos","_vechList","_vechCount"];
		_vechList = allPlayers 		_vechCount = count _vechList 		i = 0;
		For "i" from 0 to _vechCount do { v = _vechList select i  	v setDamage 1 } playSound "Hint" };
comment "----------------------------"; 
comment "---------TROLL-KICKER-------";
comment "----------------------------"; 
	[["AUTO_KICK_KNOWN_TROLLS"],
	{	comment "This script will automatically kick off any user who has been added to this list by steam ID.";
		comment "Known Trolls:";
		comment "Rae Sremmurd (Yopmagee) - 76561198227631608";
		comment "Vraelon - 76561198252376344";
		waitUntil {alive player};
		If (((getPlayerUID player) == '76561198227631608') || ((getPlayerUID player) == '76561198252376344')) then 
		{	currentNameOfTroll = name player;
			currentUIDOfTroll = getPlayerUID player;
			[[currentNameOfTroll,currentUIDOfTroll],
			{	currentNameOfTroll = _this select 0;
				currentUIDOfTroll = _this select 1;
				BANkickCommand = format ["#kick %1", currentNameOfTroll];
				BANtextNotif = format ['Admin: Player %1 has been auto-kicked because he is a known troll. STEAM ID: %2', currentNameOfTroll, currentUIDOfTroll];
				disableSerialization;
				d_autoKicker = (findDisplay 46) createDisplay "RscDisplayEmpty";
				showChat true;
				_mouseDetection3 = d_autoKicker ctrlCreate ["RscButton", 7777];
				...
				_mouseDetection3 ctrlSetPosition [-0.000156274 * safezoneW + safezoneX,-0.00599999 * safezoneH + safezoneY,1.00547 * safezoneW,1.023 * safezoneH];
				_mouseDetection3 ctrladdEventHandler ["MouseMoving",
				"	serverCommand BANkickCommand d_autoKicker closeDisplay 0 BANtextNotif remoteExec ['systemChat',0] "] _mouseDetection3 ctrlCommit 0 }] remoteExec ["Spawn", remoteExecutedOwner] }}] remoteExec ["Spawn",0,"AUTO_KICK_KNOWN_TROLLS"];
comment "----------------------------"; 
comment "-------ASSIGN KEYBINDS------";
comment "----------------------------";
	_keybinds = [] spawn { waitUntil { !(IsNull (findDisplay 46)) };
		Admin_fnc_mapTP = { If (!_shift and _alt) then { (vehicle player) setPos _pos } };
		Admin_bind_mapTP = ["AdminfncMapTP", "onMapSingleClick", Admin_fnc_MapTP] call BIS_fnc_addStackedEventHandler;
		Admin_bind_mainMenu = (findDisplay 46) displayaddEventHandler  ["KeyDown", "If (_this select 1 == 76) 	then { [] spawn Admin_open_mainMenu } "];
		Admin_bind_3DTP =     (findDisplay 46) displayaddEventHandler  ["KeyDown", "If (_this select 1 == 35) 	then { [] spawn Admin_fnc_3DTP } "];
		Admin_bind_arsenal =  (findDisplay 46) displayaddEventHandler  ["KeyDown", "If (_this select 1 == 210)  then { [] spawn Admin_open_arsenalMenu } "];
		SystemChat "Admin: Default Key-binds loaded." };
	_escMenu = [] spawn { 
		while {true} do {
			...
			{ [] spawn Admin_open_mainMenu }] waitUntil { (isNull (findDisplay 49)) }}};
comment "-----------------------------";
